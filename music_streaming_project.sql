...
-- NOTE: Run section-by-section. Safe to re-run after DROP statements.
...
CREATE TABLE Songs (
    ...
    artist_id      INT NOT NULL,
    album_id       INT,
    -- play_count will be added later via ALTER (to satisfy "alter add column" requirement)
    FOREIGN KEY (artist_id) REFERENCES Artists(artist_id),
...
ALTER TABLE Songs
ADD play_count INT NOT NULL DEFAULT 0;
...
WHERE playlist_id = 2 AND song_id = 4;
...
/* ===============================
   8) AGGREGATE FUNCTIONS + GROUP BY + HAVING
   =============================== */
...
SELECT a.artist_name, AVG(s.duration_sec) AS avg_duration_sec
FROM Songs s
JOIN Artists a ON s.artist_id = a.artist_id
...
SELECT u.user_id, u.name, play_ct
FROM (
    SELECT user_id, COUNT(*) AS play_ct
    FROM ListeningHistory
    GROUP BY user_id
) t
JOIN Users u ON u.user_id = t.user_id
WHERE t.play_ct > (SELECT AVG(cnt) FROM (SELECT COUNT(*) AS cnt FROM ListeningHistory GROUP BY user_id) x);
...
/* ===============================
   11) TRIGGER (auto-increment Songs.play_count on new listen)
   =============================== */
DELIMITER //
